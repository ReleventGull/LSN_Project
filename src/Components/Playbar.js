...
import { removeLikedTrack, likeTrack } from "./api"
const Playbar = ({trackLiked, setTrackLiked, setCurrentSongMs, navigatingSong, isPlaying, deviceVolume, setDeviceVolume, songPlaying, player, setIsPlaying, isPlayingRef, songMs, currentSongMs}) => {
    ...
    const addTrack = async () => {
        const response = await likeTrack({token: localStorage.getItem("LSNToken"), id:songPlaying.id})
        if(response.ok) {
            setTrackLiked(true)
        }
    }
    ...
    const removeTrack = async() => {
        const response = await removeLikedTrack({token: localStorage.getItem("LSNToken"), id:songPlaying.id})
        if(response.ok) {
            setTrackLiked(false)
        }
    }
            ...
            <div className="bg-white h-20">
                <div className="grid grid-cols-[1fr_1.5fr_1fr] w-full h-full bg-playbar">
                        ...
                        <div className="grid grid-cols-[auto_2fr] items-center">
                            ...
                            <div className="flex flex-row items-center">
                                ...
                                <div>
                                    {
                                        trackLiked === false ? 
                                        <svg onClick={addTrack} className="likeSongIcon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        :
                                        <svg onClick={removeTrack} className="songLikedIcon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="h-full">
                        ...
                        <input onChange={(e) => {player.setVolume(e.target.value/100), setDeviceVolume(e.target.value)}}type="range" min="1" max="100" value={deviceVolume} class="volumeSlider"/>
                        </div>
                    </div>
                </div>
        </div>
